"""
backend/stage1_3_cascade.py — Stages 1-3: cascade classification.

  Stage 1: YAMNet      — 521 AudioSet classes, biological/marine/noise signal
  Stage 2: Multispecies Whale — 12 cetacean classes (Orca, Humpback, Blue…)
  Stage 3: Humpback Whale    — specialized binary detector
...
def _get_models() -> dict:
    """Loads the 3 models once per process."""
    if _models:
        return _models

    logger.info("Loading TF models (this may take a while the first time)…")
...
def run_cascade(
    ...
    """
    Runs the cascade on a list of WAVs.
    ...
    logger.info("Stage 1-3 — cascade over %d clip(s)", len(wav_paths))
