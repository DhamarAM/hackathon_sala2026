"""
pipeline/stage4_rank.py — Stage 4: ranking multidimensional por importancia biológica.

Toma los resultados del cascade y asigna un score 0-100 a cada clip
basado en 9 dimensiones ponderadas. Produce CSV y JSON ordenados.

Dimensiones 1-7: cascade (YAMNet + Multispecies + Humpback)
Dimensión 8: ndsi_score  — índice de soundscape marino (Stage 5, opcional)
Dimensión 9: cluster_signal — señal de cluster biológico (Stage 6, opcional)

Uso:
    from pipeline.stage4_rank import rank_results

    rank_results(results_dict, output_dir=Path("pipeline_output/ranking"))
...
from pipeline.config import RANK_WEIGHTS, RANK_TIERS, RANKING_DIR

logger = logging.getLogger(__name__)


def _score(result: dict) -> dict:
    """Calcula los 7 componentes de score para un resultado."""
    ...
    # 1. Whale sustained presence: combina pico y actividad sostenida
    ...
    # 3. Acoustic diversity: distintas especies/tipos detectados
    ...
    # 5. Cross-model agreement: bonus cuando varios modelos coinciden
    cross = 0.0
    ...
    return {
        ...
        # Placeholders — sobrescritos por rank_results si soundscape/clusters presentes
        'ndsi_score':         0.0,
        'cluster_signal':     0.0,
    }
...
def rank_results(
    ...
    """
    Rankea los resultados del cascade por importancia biológica.

    Args:
        results:    Salida del cascade (Stage 1-3).
        soundscape: (opcional) Salida de Stage 5 con ndsi_underwater por clip.
        clusters:   (opcional) Salida de Stage 6 con cluster_id y dominant_band.
        output_dir: Carpeta donde se escriben ranked.csv y ranked.json.

    Returns:
        Lista de dicts ordenada por score descendente.
        Backward-compatible: si soundscape y clusters son None, funciona igual que antes.
    """
    ...
    for fname, result in results.items():
        ...
        # ── Dimensión 8: ndsi_score (Stage 5) ────────────────────────────────
        if soundscape and fname in soundscape:
            ss = soundscape[fname]
            ndsi = ss.get('ndsi_underwater', 0.0)
            # mapear [-1, 1] → [0, 1]: NDSI alto = buena biología
            components['ndsi_score'] = round(float(np.clip((ndsi + 1) / 2, 0.0, 1.0)), 4)
        # else: queda en 0.0 (del _score placeholder)

        # ── Dimensión 9: cluster_signal (Stage 6) ─────────────────────────────
        if clusters and fname in clusters:
            ...
            # bonus si está en cluster real (no outlier) y banda no es LOW
            if cid >= 0 and dom != 'LOW':
                # escalar por tamaño del cluster (más clips similares = más confianza)
                components['cluster_signal'] = round(min(1.0, 0.5 + 0.1 * csize), 4)
            elif cid >= 0:
                components['cluster_signal'] = 0.3  # cluster LOW (probable barco)
            # outlier (cid == -1): queda en 0.0
    ...
    # Log resumen
    logger.info("Stage 4 done — %d clips rankeados", len(rankings))
