"""
pipeline/stage1_3_cascade.py — Stages 1-3: clasificación en cascada.
...
Uso:
    ...
    results = run_cascade(clip_paths, output_dir=Path("pipeline_output/analysis"))
"""
...
from pipeline.config import (
    ...
    MULTISPECIES_THRESHOLD, HUMPBACK_THRESHOLD,
    WHALE_SPECIES,
...
def _classify_one(wav_path: Path, spec_dir: Path, ann_dir: Path) -> dict:
    ...
    try:
        ...
        for d in s2['detections']:
            label = 'Whale' if d['max_score'] >= 0.1 else 'Possible whale'
...
def run_cascade(
    ...
    # Guardar consolidado (sin time_series para no inflar el JSON)
    consolidated = {r: {k: v for k, v in d.items()
                        if k not in ('stage2_multispecies', 'stage3_humpback')}
                    | {'stage2_multispecies': {k2: v2 for k2, v2 in
                        d.get('stage2_multispecies', {}).items() if k2 != 'top_time_series'},
                       'stage3_humpback': {k2: v2 for k2, v2 in
                        d.get('stage3_humpback', {}).items() if k2 != 'time_series'}}
                    for r, d in results.items()}

    results_path = output_dir / "results.json"
    results_path.write_text(json.dumps({
        'pipeline': 'Cascade v2.0 (Stage 0 integrated)',
