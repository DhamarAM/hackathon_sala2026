"""
pipeline/stage0_clip.py — Stage 0: recorta WAVs a segmentos con actividad.

Soporta 16-bit y 24-bit PCM WAV. Elimina silencios antes de que los
modelos downstream procesen el audio, mejorando la señal/ruido.

Uso:
    from pipeline.stage0_clip import AudioClipper

    clipper = AudioClipper(source_dir=Path("data/raw"), output_dir=Path("pipeline_output/clips"))
...
class AudioClipper:
    """
    Recorre una carpeta de WAVs, detecta segmentos activos por RMS
    y escribe clips precisos en output_dir.

    Soporta 16-bit y 24-bit PCM. Procesa archivos de cualquier sr.
    """
    ...
    def run(self) -> List[Path]:
        """Procesa todos los WAVs en source_dir. Devuelve lista de clips escritos."""
    ...
    def _decode(raw: bytes, sample_width: int, n_channels: int) -> np.ndarray:
        if sample_width == 2:
            ...
            # 24-bit little-endian → int32 con extensión de signo
    ...
    def _find_segments(
        ...
        # Filtrar cortos y convertir a frames
    ...
    def _write_clip(src: wave.Wave_read, start_frame: int, n_frames: int, dst: Path) -> None:
        ...
        # Pad con ceros si el clip es más corto que el mínimo del modelo multispecies
