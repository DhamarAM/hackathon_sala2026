"""
pipeline/run.py — Entrypoint principal del pipeline integrado.
...
Uso:
    python -m pipeline.run --source <carpeta_con_wavs>

    # Opciones:
    python -m pipeline.run --source data/raw --output pipeline_output
    python -m pipeline.run --source data/raw --skip-clip      # usa clips previos
    python -m pipeline.run --source data/raw --skip-cascade   # solo rankea
    python -m pipeline.run --source data/raw --no-cluster     # sin Stage 6 (sin GPU)
    python -m pipeline.run --skip-clip --skip-cascade --no-cluster  # solo Stages 5+4
"""
...
def run(
    ...
    """
    from pipeline.stage0_clip      import AudioClipper
    from pipeline.stage1_3_cascade import run_cascade
    from pipeline.stage4_rank      import rank_results
    from pipeline.stage5_soundscape import run_soundscape
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Ejemplos:
  python -m pipeline.run --source data/raw_data
  python -m pipeline.run --source hackathon_data/marine-acoustic/6478
  python -m pipeline.run --source data/raw --skip-clip
  python -m pipeline.run --skip-clip --skip-cascade --no-cluster   # solo Stage 5+4
  python -m pipeline.run --skip-clip --skip-cascade --skip-cluster  # reusar Stage 6 también
        """,
    )
    default_source = Path(__file__).resolve().parent.parent / "hackathon_data" / "marine-acoustic"
    default_output = Path(__file__).resolve().parent.parent / "pipeline_output"
    ...
    parser.add_argument('--output',          default=default_output, type=Path,
                        help='Carpeta de salida (default: pipeline_output/)')
